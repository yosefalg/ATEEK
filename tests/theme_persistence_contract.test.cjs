...
test('theme preference reads and writes are serialized and failure-safe', () => {
  ...
  assert.match(source, /Promise\.resolve\(\)\.then\(\(\)=>AsyncStorage\.multiGet\(\[\.\.\.STARTUP_KEYS\]\)\)\.then\(entries=>/);
  ...
  assert.doesNotMatch(source, /void AsyncStorage\.multiGet\(/);
