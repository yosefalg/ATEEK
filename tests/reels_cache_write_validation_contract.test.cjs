...
test('reels cache read validation rejects unreferenced or non-canonical listing keys and cleans the persisted snapshot', () => {
  const cache = fs.readFileSync('src/services/reelsCache.ts', 'utf8');
  ...
  assert.match(helperSection, /isCanonicalId\(listingId\)/);
  assert.match(helperSection, /Object\.keys\(listings\)\.every\(\(listingId\) => isCanonicalId\(listingId\) && referencedIds\.has\(listingId\)\)/);
...
test('reels cache persists listing payloads only for canonical keys referenced by the bounded reels snapshot', () => {
  const cache = fs.readFileSync('src/services/reelsCache.ts', 'utf8');
  const writeSection = cache.slice(cache.indexOf('export async function writeReelsSnapshot'));
  const referencedIdsIndex = writeSection.indexOf('const boundedListingIds = new Set(boundedReels.map');
  const filterIndex = writeSection.indexOf('Object.entries(listings).filter(([listingId]) => isCanonicalId(listingId) && boundedListingIds.has(listingId))');
  ...
  assert.ok(filterIndex > referencedIdsIndex, 'listing payloads must be filtered by canonical referenced bounded reel ids');
...
test('empty reels snapshots cannot retain stale listing payloads and keep a null cursor', () => {
  ...
  assert.match(writeSection, /Object\.entries\(listings\)\.filter\(\(\[listingId\]\) => isCanonicalId\(listingId\) && boundedListingIds\.has\(listingId\)\)/);
