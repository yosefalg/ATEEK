...
test('search transform defers expensive listing filtering without deferring username routing', () => {
  ...
  assert.ok(source.includes("if(query.trim().startsWith('@'))return[];"), 'username mode must continue switching immediately from the live query');
  assert.ok(source.includes("[searchableListings,category,query,deferredQuery,near,sort]"), 'memo dependencies must track both immediate routing and deferred filtering state');
});
