...
patch('src/components/SpatialReelsHub.tsx', [
  ...
  ['52px action rail', s => s.replace(/rail:\{([^}]*)\}/, (_m, body) => `rail:{${body},...{width:52,gap:16,alignItems:'center'}}`)],
]);
...
patch('src/components/SpatialDealScreens.tsx', [
  ...
  ['bubble max width', s => s.replace(/bubble:\s*\{([^}]*)\}/, (_m, body) => `bubble: {${body}, ...{ maxWidth: '78%' }}`)],
  ['reference card fixed height', s => s.replace(/contextBanner:\s*\{([^}]*)\}/, (_m, body) => `contextBanner: {${body}, ...{ height: 56, minHeight: 56, maxHeight: 56, overflow: 'hidden' }}`)],
  ['message list top anchor', s => s.replace(/messagesContent:\s*\{([^}]*)\}/, (_m, body) => `messagesContent: {${body}, ...{ paddingTop: 64 }}`)],
]);

patch('src/components/SpatialProfileAnalyticsHub.tsx', [
  ['standard page padding', s => s.replace(/page:\s*\{([^}]*)\}/, (_m, body) => `page: {${body}, ...{ paddingHorizontal: 16, gap: 12 }}`)],
  ['metric glass clipping', s => s.replace(/metric:\s*\{([^}]*)\}/, (_m, body) => `metric: {${body}, ...{ borderRadius: 16, overflow: 'hidden' }}`)],
  ['panel glass clipping', s => s.replace(/panel:\s*\{([^}]*)\}/, (_m, body) => `panel: {${body}, ...{ borderRadius: 16, overflow: 'hidden' }}`)],
  ['24 icon frame', s => s.replace(/settingIcon:\s*\{([^}]*)\}/, (_m, body) => `settingIcon: {${body}, ...{ width: 24, height: 24, minWidth: 24, alignItems: 'center', justifyContent: 'center' }}`)],
  ['setting row switch anchor', s => s.replace(/settingRow:\s*\{([^}]*)\}/, (_m, body) => `settingRow: {${body}, ...{ alignItems: 'center', justifyContent: 'space-between' }}`)],
]);

console.log('Run #57 visual refinement applied deterministically.');
