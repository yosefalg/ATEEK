...
cloud=replace(
  ...
  "const appStateSub=AppState.addEventListener('change',(state)=>{if(state==='active')void refresh();});const timer=setInterval(()=>{if(AppState.currentState==='active')void refresh();},lowData?60000:15000);return()=>{alive.current=false;appStateSub.remove();clearInterval(timer);void supabase.removeChannel(channel);};",
