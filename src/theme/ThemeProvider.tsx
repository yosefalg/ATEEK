...
type Ctx={mode:ThemeMode;resolved:'dark'|'light';colors:ThemeColors;setMode:(m:ThemeMode)=>void;toggle:()=>void;lowData:boolean;setLowData:(v:boolean)=>void};
const ThemeContext=createContext<Ctx>({mode:'dark',resolved:'dark',colors:dark,setMode:()=>{},toggle:()=>{},lowData:false,setLowData:()=>{}});
const THEME_KEY='ateek.theme.mode',LOW_DATA_KEY='ateek.low.data';
export function ThemeProvider({children}:{children:PropsWithChildren['children']}){const system=useColorScheme();const[mode,setModeState]=useState<ThemeMode>('dark');const[lowData,setLowDataState]=useState(false);useEffect(()=>{void Promise.all([AsyncStorage.getItem(THEME_KEY),AsyncStorage.getItem(LOW_DATA_KEY)]).then(([m,l])=>{if(m==='dark'||m==='light'||m==='system')setModeState(m);setLowDataState(l==='1')}).catch(()=>{})},[]);const setMode=(m:ThemeMode)=>{setModeState(m);void AsyncStorage.setItem(THEME_KEY,m)};const setLowData=(v:boolean)=>{setLowDataState(v);void AsyncStorage.setItem(LOW_DATA_KEY,v?'1':'0')};const resolved=mode==='system'?(system==='light'?'light':'dark'):mode;const value=useMemo<Ctx>(()=>({mode,resolved,colors:resolved==='dark'?dark:light,setMode,toggle:()=>setMode(resolved==='dark'?'light':'dark'),lowData,setLowData}),[mode,resolved,lowData]);return <ThemeContext.Provider value={value}>{children}</ThemeContext.Provider>}
