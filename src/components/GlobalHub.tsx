      ...
      <View style={[s.backdrop,{backgroundColor:colors.overlay}]}>
        <View style={[s.sheet,{backgroundColor:colors.surface,borderColor:colors.line,direction:isRTL?'rtl':'ltr'}]}>
          ...
          <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={s.scroll} keyboardShouldPersistTaps="handled">
            ...
            <View style={s.row}>{(['ar','ku','en'] as const).map(x=><Pressable key={x} onPress={()=>{haptics.tap();setLocale(x)}} style={[s.pill,{backgroundColor:locale===x?colors.gold:colors.glass,borderColor:locale===x?colors.gold:colors.line}]}><Text style={{color:locale===x?colors.forest:colors.ink,fontWeight:'900'}}>{x==='ar'?'العربية':x==='ku'?'کوردی':'English'}</Text></Pressable>)}</View>
