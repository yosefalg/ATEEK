...
export function ListingCard({ item, favorite, onFavorite, onPress }: Props) {
  ...
  return (
    <Animated.View style={[styles.wrap, motion]}>
      <Pressable
        accessibilityRole="button"
        accessibilityLabel={`${item.title}، ${formatPrice(item.price)}، ${item.location}`}
        accessibilityHint="يفتح تفاصيل الإعلان"
        onPressIn={down}
        onPressOut={up}
        onPress={onPress}
        style={[styles.card, { backgroundColor: colors.glass, borderColor: colors.line }]}
      >
        ...
        <View style={[styles.imageWrap, { backgroundColor: colors.forestSoft }]}>
          ...
          {item.sellerId && (
            <Pressable
              accessibilityRole="button"
              accessibilityLabel="فتح بروفايل البائع"
              accessibilityHint="يفتح صفحة البائع دون فتح الإعلان"
              onPress={event => {
                event.stopPropagation();
                haptics.light();
        ...
        <View style={styles.body}>
          ...
          <Text style={[styles.price, { color: colors.gold }]}>{formatPrice(item.price)}</Text>
          {metrics && (metrics.activeSeller || metrics.completedDeals > 0 || metrics.newAccount) && (
            <View style={styles.trustRow}>
              {metrics.activeSeller && <Text style={[styles.trust, { color: colors.success }]}>● بائع نشط</Text>}
              {metrics.completedDeals > 0 && <Text style={[styles.trust, { color: colors.cyan }]}>{metrics.completedDeals} صفقة</Text>}
              {metrics.newAccount && <Text style={[styles.trust, { color: colors.gold }]}>حساب جديد</Text>}
            </View>
          )}
          <View style={styles.meta}>
            ...
            <Text style={[styles.metaText, { color: colors.muted }]}>{item.age}</Text>
          </View>
        </View>
      </Pressable>
    </Animated.View>
...
const styles = StyleSheet.create({
  wrap: { width: '48.3%', marginBottom: 14 },
  card: { width: '100%', borderRadius: 24, overflow: 'hidden', borderWidth: 1, ...shadows.card },
  ...
  profileButton: { position: 'absolute', right: 9, bottom: 9, width: 34, height: 34, borderRadius: 13, borderWidth: 1, alignItems: 'center', justifyContent: 'center' },
  body: { padding: 12, alignItems: 'flex-end' },
  title: { fontSize: 14, fontWeight: '900', writingDirection: 'rtl', width: '100%', textAlign: 'right' },
  price: { fontSize: 16, fontWeight: '900', marginTop: 6 },
  trustRow: { width: '100%', flexDirection: 'row-reverse', flexWrap: 'wrap', gap: 7, marginTop: 7 },
  trust: { fontSize: 9, fontWeight: '900' },
