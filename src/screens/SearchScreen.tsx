  ...
  return <View style={styles.root}>
    <View style={[styles.header,{backgroundColor:colors.glassStrong,borderBottomColor:colors.line}]}>
      ...
      <View style={[styles.search,{backgroundColor:colors.glass,borderColor:colors.line}]}>
        ...
        <Pressable accessibilityRole="button" accessibilityLabel="تنفيذ البحث" accessibilityState={{busy:userBusy,disabled:userBusy}} disabled={userBusy} hitSlop={8} onPress={()=>void submit()}><Ionicons name={query.trim().startsWith('@')?'person-circle-outline':'search'} size={22} color={query.trim().startsWith('@')?colors.cyan:colors.muted}/></Pressable>
      </View>
      {query.trim().startsWith('@')&&<Text accessibilityLiveRegion="polite" style={[styles.usernameHint,{color:colors.cyan}]}>{userBusy?'جارٍ فتح البروفايل…':'اضغط بحث لفتح البروفايل العام مباشرة'}</Text>}
    ...
    <FlatList data={filtered} keyExtractor={x=>x.id} numColumns={2} initialNumToRender={lowData?4:8} maxToRenderPerBatch={lowData?4:8} windowSize={lowData?3:6} removeClippedSubviews columnWrapperStyle={styles.row} contentContainerStyle={styles.results} keyboardShouldPersistTaps="handled" ListEmptyComponent={query.trim().startsWith('@')?<EmptyState icon="person-circle-outline" title="بحث بروفايل" body="اكتب @username واضغط بحث لفتح الحساب مباشرة"/>:<EmptyState icon="search-outline" title="لم نجد نتائج" body="جرّب كلمة أخرى أو اختر قسمًا مختلفًا"/>} renderItem={({item})=><ListingCard item={item} favorite={favorites.includes(item.id)} onFavorite={()=>onFavorite(item.id)} onPress={()=>{void commitHistory(query);onOpen(item);}}/>}/>
  </View>;
}
