...
const parseSaved=(raw:string|null)=>{if(!raw)return[];try{const value=JSON.parse(raw);if(!Array.isArray(value))return[];const seen=new Set<string>(),next:Saved[]=[];for(const item of value){if(!item||typeof item.q!=='string'||typeof item.category!=='string')continue;const q=item.q.trim(),category=item.category.trim();if(!categoryOptions.some(x=>x.id===category)||q.startsWith('@')||(!q&&category==='all'))continue;const key=JSON.stringify([normalize(q),category]);if(seen.has(key))continue;seen.add(key);next.push({q,category});if(next.length===12)break}return next}catch{return[]}};
  ...
  return <View style={styles.root}>
    <View style={[styles.header,{backgroundColor:colors.glassStrong,borderBottomColor:colors.line}]}>
      ...
      <View style={[styles.search,{backgroundColor:colors.glass,borderColor:colors.line}]}>
        ...
        <TextInput accessibilityLabel="حقل البحث" accessibilityHint="ابحث عن سلعة أو اكتب معرف مستخدم يبدأ بعلامة @" autoFocus value={query} onChangeText={changeQuery} onSubmitEditing={()=>void submit()} placeholder="سلعة أو @username" placeholderTextColor={colors.muted} autoCapitalize="none" autoCorrect={false} maxLength={120} returnKeyType="search" style={[styles.input,{color:colors.ink}]}/>
        {!!query&&<Pressable accessibilityRole="button" accessibilityLabel="مسح عبارة البحث" accessibilityHint="يمسح النص الحالي ويلغي أي بحث بروفايل جارٍ" hitSlop={8} onPress={()=>changeQuery('')}><Ionicons name="close-circle" size={20} color={colors.muted}/></Pressable>}
