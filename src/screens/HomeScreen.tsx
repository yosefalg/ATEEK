...
export function HomeScreen({listings,favorites,onFavorite,onOpen,onNavigate,notificationCount,unreadChats,activeListings,profile}:Props){
  ...
  return <ScrollView style={s.root} contentContainerStyle={s.content} showsVerticalScrollIndicator={false}>
    <View style={s.topBar}>
      ...
      <View style={s.centerCell}><Pressable accessibilityRole="button" accessibilityLabel="الإشعارات" onPress={()=>onNavigate('notifications')} style={[s.iconButton,{borderColor:colors.line,backgroundColor:colors.glass}]}><Ionicons name="notifications-outline" size={24} color={colors.ink}/>{notificationCount>0&&<View style={s.badge}><Text style={s.badgeText}>{Math.min(notificationCount,99)}</Text></View>}</Pressable></View>
      <View style={s.profileCell}><Pressable accessibilityRole="button" accessibilityLabel="الحساب" onPress={()=>onNavigate('profile')} style={[s.avatar,{borderColor:colors.line,backgroundColor:colors.glass}]}>{profile?.avatar_url?<Image source={{uri:String(profile.avatar_url)}} style={StyleSheet.absoluteFill}/>:<Ionicons name="person" size={24} color={colors.gold}/>}</Pressable></View>
    </View>
    ...
    <View style={s.quickGrid}>{quick.map(q=><Pressable key={q.label} accessibilityRole="button" accessibilityLabel={q.label} onPress={()=>onNavigate(q.tab)} style={[s.quickCard,{backgroundColor:colors.glass,borderColor:colors.line}]}><View style={[s.quickIcon,{borderColor:colors.line}]}><Ionicons name={q.icon} size={28} color={colors.gold}/></View><View style={s.quickCopy}><Text style={[s.quickLabel,{color:colors.ink}]}>{q.label}</Text>{q.value!=null&&<Text style={[s.quickValue,{color:colors.muted}]}>{q.value}</Text>}</View></Pressable>)}</View>

    <View style={s.sectionHeader}><Text style={[s.sectionTitle,{color:colors.ink}]}>أحدث الإعلانات</Text><Pressable onPress={()=>onNavigate('search')} accessibilityLabel="عرض كل الإعلانات"><Ionicons name="arrow-back" size={20} color={colors.gold}/></Pressable></View>
    <FlatList horizontal inverted data={latestListings} keyExtractor={x=>x.id} showsHorizontalScrollIndicator={false} initialNumToRender={lowData?2:4} maxToRenderPerBatch={lowData?2:4} contentContainerStyle={s.horizontal} renderItem={({item})=><Pressable accessibilityRole="button" accessibilityLabel={item.title} onPress={()=>onOpen(item)} style={[s.listingCard,{backgroundColor:colors.glass,borderColor:colors.line}]}>{item.image?<Image source={{uri:item.image}} style={s.listingImage}/>:<View style={[s.listingImage,s.placeholder]}><Ionicons name="image-outline" size={28} color={colors.muted}/></View>}<View style={s.listingBody}><Text numberOfLines={1} style={[s.listingTitle,{color:colors.ink}]}>{item.title}</Text><Text style={[s.price,{color:colors.gold}]}>{money(item.price)}</Text><Pressable accessibilityLabel="المفضلة" hitSlop={10} onPress={()=>onFavorite(item.id)} style={s.favorite}><Ionicons name={favorites.includes(item.id)?'heart':'heart-outline'} size={20} color={favorites.includes(item.id)?'#FF7B86':colors.muted}/></Pressable></View></Pressable>} ListEmptyComponent={<View style={[s.empty,{borderColor:colors.line}]}><Text style={[s.emptyText,{color:colors.muted}]}>لا توجد إعلانات حالياً</Text></View>}/>

    <View style={s.sectionHeader}><Text style={[s.sectionTitle,{color:colors.ink}]}>أحدث الريلز</Text><Pressable onPress={()=>onNavigate('ai')} accessibilityLabel="فتح الريلز"><Ionicons name="play-circle-outline" size={24} color={colors.gold}/></Pressable></View>
    <FlatList horizontal inverted data={reels} keyExtractor={x=>x.id} showsHorizontalScrollIndicator={false} initialNumToRender={lowData?1:3} maxToRenderPerBatch={lowData?1:3} contentContainerStyle={s.horizontal} renderItem={({item})=><Pressable accessibilityRole="button" accessibilityLabel={item.caption||'ريل'} onPress={()=>onNavigate('ai')} style={[s.reelCard,{backgroundColor:colors.glass,borderColor:colors.line}]}>{item.thumbnail_url?<Image source={{uri:item.thumbnail_url}} style={s.reelImage}/>:<View style={[s.reelImage,s.placeholder]}><Ionicons name="play" size={30} color={colors.gold}/></View>}<Text numberOfLines={2} style={[s.reelCaption,{color:colors.ink}]}>{item.caption||'ريل جديد'}</Text></Pressable>} ListEmptyComponent={<View style={[s.empty,{borderColor:colors.line}]}><Text style={[s.emptyText,{color:colors.muted}]}>لا توجد ريلز حالياً</Text></View>}/>
  </ScrollView>;
}
