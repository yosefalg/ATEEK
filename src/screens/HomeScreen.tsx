...
export function HomeScreen({listings,favorites,onFavorite,onOpen,onNavigate,notificationCount,unreadChats,activeListings,profile}:Props){
  ...
  const profileAvatar=safeRemoteMediaUrl(firstNonEmpty(profile?.avatar_url))??'';
  const quick=[
  ...
  return <ScrollView style={s.root} contentContainerStyle={s.content} showsVerticalScrollIndicator={false}>
    ...
    <FlatList horizontal inverted data={reels} keyExtractor={x=>x.id} showsHorizontalScrollIndicator={false} initialNumToRender={lowData?1:3} maxToRenderPerBatch={lowData?1:3} contentContainerStyle={s.horizontal} renderItem={({item})=>{const thumbnail=safeRemoteMediaUrl(firstNonEmpty(item.thumbnail_url))??'';return <Pressable accessibilityRole="button" accessibilityLabel={item.caption||'ريل'} accessibilityHint="يفتح الريلز" onPress={()=>onNavigate('ai')} style={[s.reelCard,{backgroundColor:colors.glass,borderColor:colors.line}]}>{thumbnail&&!failedImageUrls.has(thumbnail)?<Image accessible={false} source={{uri:thumbnail}} style={s.reelImage} onError={()=>markImageFailed(thumbnail)}/>:<View style={[s.reelImage,s.placeholder,{backgroundColor:colors.forestSoft}]}><Ionicons name="play" size={30} color={colors.gold}/></View>}<Text numberOfLines={2} style={[s.reelCaption,{color:colors.ink}]}>{item.caption||'ريل جديد'}</Text></Pressable>}} ListEmptyComponent={<View style={[s.empty,{borderColor:colors.line,backgroundColor:colors.glass}]}><Text style={[s.emptyText,{color:colors.muted}]}>لا توجد ريلز حالياً</Text></View>}/>
  </ScrollView>;
}
...
const s=StyleSheet.create({
  ...
  sectionHeader:{marginTop:6,minHeight:44,flexDirection:'row-reverse',alignItems:'center',justifyContent:'space-between'},sectionTitle:{fontFamily:'System',fontSize:17,fontWeight:'900',textAlign:'right'},sectionAction:{minHeight:44,borderRadius:12,borderWidth:1,paddingHorizontal:12,flexDirection:'row-reverse',alignItems:'center',gap:4},sectionActionText:{fontFamily:'System',fontSize:10,fontWeight:'800'},horizontal:{gap:12,paddingVertical:4},
