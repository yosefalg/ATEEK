...
export function HomeScreen({listings,favorites,onFavorite,onOpen,onNavigate,notificationCount,unreadChats,activeListings,profile}:Props){
  ...
  return <ScrollView style={s.root} contentContainerStyle={s.content} showsVerticalScrollIndicator={false}>
    <View style={s.topBar}>
      ...
      <View style={s.profileCell}><Pressable accessibilityRole="button" accessibilityLabel="الحساب" accessibilityHint="يفتح ملفك وإعدادات الحساب" onPress={()=>onNavigate('profile')} style={[s.avatar,{borderColor:colors.line,backgroundColor:colors.glass}]}>{profileAvatar&&!failedImageUrls.has(profileAvatar)?<Image source={{uri:profileAvatar}} style={StyleSheet.absoluteFill} onError={()=>markImageFailed(profileAvatar)}/>:<Ionicons name="person" size={24} color={colors.gold}/>}</Pressable></View>
    </View>
    ...
    <FlatList horizontal inverted data={reels} keyExtractor={x=>x.id} showsHorizontalScrollIndicator={false} initialNumToRender={lowData?1:3} maxToRenderPerBatch={lowData?1:3} contentContainerStyle={s.horizontal} renderItem={({item})=>{const thumbnail=item.thumbnail_url??'';return <Pressable accessibilityRole="button" accessibilityLabel={item.caption||'ريل'} accessibilityHint="يفتح الريلز" onPress={()=>onNavigate('ai')} style={[s.reelCard,{backgroundColor:colors.glass,borderColor:colors.line}]}>{thumbnail&&!failedImageUrls.has(thumbnail)?<Image source={{uri:thumbnail}} style={s.reelImage} onError={()=>markImageFailed(thumbnail)}/>:<View style={[s.reelImage,s.placeholder,{backgroundColor:colors.forestSoft}]}><Ionicons name="play" size={30} color={colors.gold}/></View>}<Text numberOfLines={2} style={[s.reelCaption,{color:colors.ink}]}>{item.caption||'ريل جديد'}</Text></Pressable>}} ListEmptyComponent={<View style={[s.empty,{borderColor:colors.line,backgroundColor:colors.glass}]}><Text style={[s.emptyText,{color:colors.muted}]}>لا توجد ريلز حالياً</Text></View>}/>
  </ScrollView>;
}
