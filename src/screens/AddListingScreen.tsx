...
export function AddListingScreen({ onAdd, onDone }: { onAdd: (item: Listing) => Promise<void>; onDone: () => void }) {
  ...
  useEffect(() => {
    const timer = setTimeout(() => {
      ...
      if (title || price || description || image) void AsyncStorage.setItem(DRAFT_KEY, JSON.stringify(draft)).catch(() => {});
    }, 5000);
  ...
  const publish = async () => {
    ...
    if (!title.trim() || !amount || !image) {
      Alert.alert('أكمل بيانات الإعلان', 'أضف صورة وعنوانًا وسعرًا صحيحًا.');
      return;
    }
    setPublishing(true);
    try {
  ...
  return (
    <KeyboardAvoidingView style={styles.root} behavior={Platform.OS === 'ios' ? 'padding' : undefined}>
      <ScrollView contentContainerStyle={styles.content} keyboardShouldPersistTaps="handled">
        ...
        >
          {image ? <Image source={{ uri: image }} style={styles.preview} accessibilityIgnoresInvertColors /> : <>
        ...
        >
          <Ionicons name="sparkles" size={21} color={colors.gold} importantForAccessibility="no" />
        ...
        <Pressable
          ...
          accessibilityState={{ disabled: busy, busy: publishing }}
        >
          <Ionicons name="paper-plane" size={19} color={colors.forest} importantForAccessibility="no" />
...
const styles = StyleSheet.create({
  ...
  subtitle: { fontSize: 12, color: colors.muted, marginTop: 4, textAlign: 'right' },
  ...
  categoryActive: { backgroundColor: colors.forest, borderColor: colors.forest },
